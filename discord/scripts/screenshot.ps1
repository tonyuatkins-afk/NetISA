# scripts/screenshot.ps1 - Capture DISCORD as PNG.

param(
    [string]$Conf       = "$PSScriptRoot\screenshot.conf",
    [string]$OutFile    = "C:\Development\Screenshots\discord_main.png",
    [int]   $BootDelay  = 5,
    [string]$Tag        = "",
...
$discordDir = Resolve-Path "$PSScriptRoot\.."
$discordExe = Join-Path $discordDir "DISCORD.EXE"
if (-not (Test-Path $discordExe)) {
    Write-Error "DISCORD.EXE not found at $discordExe. Run wmake in $discordDir first."
    exit 1
}
